...
const rendertemplateapp = () => {
  const template = (
    <div>
      ...
      <p>
        {templateapp.options.length > 0 ? "Here is your options" : "No options"}
      </p>
      <button onClick={onMakeTask} disabled={templateapp.options.length == 0}>
        What should i do
      </button>
      <button onClick={removeAll}>Remove All</button>
      <ol>
        {templateapp.options.map((num, i) => {
          return <li>{templateapp.options[i]}</li>;
        })}
      </ol>
